declare const Telegram: ITelegram;
...
interface ITelegram {
  Utils: {
    ...
    urlAppendHashParams: (url, addHash) => any;
    urlParseHashParams: (locationHash) => any;
    urlParseQueryString: (queryString) => any;
    urlSafeDecode: (urlencoded) => any;
  };
  WebView: {
    callEventCallbacks: (eventType, func) => any;
    initParams: { tgWebAppVersion: '7.0'; tgWebAppPlatform: 'macos'; tgWebAppThemeParams: '' };
    isIframe: boolean;
    offEvent: (eventType, callback) => any;
    onEvent: (eventType, callback) => any;
    postEvent: (eventType, callback, eventData) => any;
    receiveEvent: (eventType, eventData) => any;
  };
  WebApp: WebApp;
}
...
interface WebApp {
  ...
  setHeaderColor(color: 'bg_color' | 'secondary_bg_color' | (string & any)): void;
  /**
   ...
   */
  setBackgroundColor(color: 'bg_color' | 'secondary_bg_color' | (string & any)): void;
  /**
